...
- 件名: `【<サイト名>】お問い合わせを受け付けました（<問い合わせ番号>）`
- 本文: 問い合わせ番号・受付日時・送信者名・メールアドレス・対象商品・本文、管理画面のお問い合わせ詳細へのリンク

**3. お問い合わせ控え（送信者宛）**

- 件名: `【<サイト名>】お問い合わせを承りました（<問い合わせ番号>）`
- 本文: 問い合わせ番号、3営業日以内に返信する旨、送信内容（対象商品・本文）の控え。番号は問い合わせを特定する手段として案内する
...
- お問い合わせ受付通知に管理画面のお問い合わせ詳細へのリンクが載る: `tests/Feature/Mail/MailRenderingTest.php`（`管理者宛のお問い合わせ通知が描画できる`）
- 受付通知と控えの双方に問い合わせ番号が載る: 同上（`管理者宛のお問い合わせ通知が描画できる`・`お問い合わせの控えが描画できる`）
