...
- 明細の商品名とサムネイルは、商品が今も購入できる場合に限り商品詳細へのリンクにする。削除済み・非公開の商品はスナップショットの文字と画像をそのまま出す（遷移先が無いため）。
...
type Props = {
    order: {
        ...
        items: {
            ...
            product_url: string | null;  // 商品詳細へのリンク先。削除済み・非公開の商品は null
...
2. 表示する値は `orders` / `order_items` のスナップショット列だけで組み立てる（`users` は参照しない）。
3. 明細のリンク先だけは現在の公開状態に依存するため、`order_items.product_id` から `products` の `id` と `is_published` のみを eager load し、公開中なら商品詳細のURLを、それ以外は `null` を渡す。商品が削除済みなら `product_id` は `null` になっているため参照は発生しない。
4. 変更履歴は古い順に並べ、遷移後のステータスと日時のみを出す（操作した管理者名は渡さない）。
...
- 公開中の商品の明細には商品詳細へのリンクが付く: 同上（`公開中の商品の明細には商品詳細へのリンクが付く`）
- 削除済み・非公開の商品の明細にはリンクが付かない: 同上（`削除済みの商品の明細にはリンクが付かない`・`非公開になった商品の明細にはリンクが付かない`）
- 明細が増えてもクエリ数が変わらない: 同上（`明細が増えても注文詳細のクエリ数は変わらない`）
