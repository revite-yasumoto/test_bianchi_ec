...
**注文を確定する（`PlaceOrderService`。1〜11を1トランザクションで囲む）**
...
12. トランザクションを抜けたあとで、会員へ注文受付メールを、管理者へ注文通知を送る。送信の仕様は [docs/mail-notification.md](../mail-notification.md) が正本。
...
- 注文の確定で会員と管理者にメールが送られ、送信に失敗しても注文が成立する: `tests/Feature/Front/Order/PlaceOrderTest.php`（`注文を確定すると会員と管理者にメールが送られる`・`メールの送信に失敗しても注文は成立する`・`注文が失敗したときはメールが送られない`）
