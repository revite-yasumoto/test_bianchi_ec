...
本ドキュメントは商品・会員・管理者マスタの**CSVフォーマット**、**文字コードの扱い**、**インポートの適用方針**の正本。各マスタの仕様書はこれらを再掲せず本書へリンクする。
...
- **アクセス権限・ミドルウェア:** 上表の通り。
- **本ドキュメントのスコープ:** 3マスタのCSV取り込みと書き出し。注文のCSVと商品画像のCSV取り込みは対象外。
...
エクスポートの列は A〜F ＋「登録日」で、**初期パスワードは含めない**。
