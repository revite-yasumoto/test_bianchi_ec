...
- ステータスはバッジで表示する（有効＝緑系、休会＝赤系、退会＝グレー系）。
...
- 管理画面から設定できるステータスは有効・休会の2つに限る。退会は会員自身の操作でのみ設定され（[docs/front/withdrawal.md](../front/withdrawal.md) が正本）、管理者が付けることも戻すこともできない。
- 退会済みの会員は一覧・詳細に表示するが、ステータス更新の操作は出さない。画面を経ない直接のリクエストも `FormRequest` で拒否する。
- ステータス更新で受け付ける値は有効・休会のみとし、退会は受け付けない。
- 休会・退会のいずれもフロントにログインできなくなる（判定は [docs/front/auth.md](../front/auth.md) が正本）。
...
- 退会済みの会員のステータスは変更できない: 同上（`退会済みの会員はステータスを更新できない`）
- 退会への変更は受け付けない: 同上（`退会へのステータス変更は受け付けない`）
