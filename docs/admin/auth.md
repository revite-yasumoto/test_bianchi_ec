...
| GET | `/admin` | `admin.home` | `auth:admin` |
| POST | `/admin/logout` | `admin.logout` | `auth:admin` |

- **アクセス権限・ミドルウェア:** 上表の通り。以降の単位が追加する管理画面ルートは全て `auth:admin` グループ内に置く。
- **本ドキュメントのスコープ:** 管理者ログイン・ログアウトのバックエンド処理、ログイン画面（`Login.tsx`）、およびログイン後の暫定遷移先である `admin.home`（`Home.tsx`）。ログイン後に使う管理画面共通レイアウト（サイドバー・ヘッダー・共通UIコンポーネント）は [docs/admin/common-layout.md](common-layout.md) が正本。
...
5. 成功時はレート制限をクリアし、セッションを再生成（`session()->regenerate()`）した上で `AuthenticatedSessionController::landingUrl()` の判定先へリダイレクトする。

**ログイン後の暫定ホーム画面（`HomeController::index()`）:** `GET /admin`（`auth:admin`）で `admin/Home` を返す。単位07・単位12が未実装の間、ログイン成功後は必ずこの画面に遷移する（内容は「ログインしました。各機能は今後の単位で実装されます。」という案内のみ）。
...
| Page | `resources/js/admin/Pages/Home.tsx` |
...
- ログイン済みは `admin.home`（`GET /admin`）を表示できる: `tests/Feature/Admin/HomeTest.php`（`admin_can_view_home`）
- 未ログインは `admin.home` アクセス時にログイン画面へリダイレクトされる: 同上（`guest_is_redirected_to_login`）
